...
Item { // Notification item area
    ...
    function processNotificationBody(body, appName) {
        ...
        if (appName) {
            ...
            // Transforma markdown da notificação do discord
            if(lowerApp === 'discord') {
                // tem que seguir a porra da ordem
                // 1) Strike through
                processedBody = processedBody.replace(/~~([\s\S]+?)~~/g, '<s>$1</s>');
                
                // 2) Negrito + Italico
                processedBody = processedBody.replace(/\*\*\*([\s\S]+?)\*\*\*/g, '<b><i>$1</i></b>');
                processedBody = processedBody.replace(/___([\s\S]+?)___/g, '<b><i>$1</i></b>');

                // 3) Negrito
                processedBody = processedBody.replace(/\*\*([\s\S]+?)\*\*/g, '<b>$1</b>');
                
                // 4) Underline
                processedBody = processedBody.replace(/__([\s\S]+?)__/g, '<u>$1</u>'); 

                // 5) Italico
                processedBody = processedBody.replace(/\*([^\*\n][\s\S]*?[^\*\n])\*/g, '<i>$1</i>');
                processedBody = processedBody.replace(/_([^_\n][\s\S]*?[^_\n])_/g, '<i>$1</i>');
            }
        }
    ...
    Rectangle { // Background of notification item
        ...
        color: (expanded && !onlyNotification) ? 
            (notificationObject.urgency == NotificationUrgency.Critical) ? 
                Appearance.colors.notifications.backgroundUrgent :
                Appearance.colors.notifications.background : Appearance.colors.notifications.backgroundSingle
        ...
        ColumnLayout { // Content column
            ...
            RowLayout { // Summary row
                ...
                StyledText {
                    ...
                    color: Appearance.colors.notifications.summary
                ...
                StyledText {
                    ...
                    font.pixelSize: root.fontSize
                    color: Appearance.colors.notifications.body
                    elide: Text.ElideRight
